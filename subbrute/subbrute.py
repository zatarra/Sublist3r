        ...
        while looking_for_wildcards and test_counter >= 0:
            ...
            try:
                ...
                if type(e) == dns.resolver.NXDOMAIN or type(e) == dns.name.EmptyLabel or type(e) == dns.resolver.NoAnswer:
                    ...
                    trace("wildcard exception:",
                          self.resolver.nameservers, type(e))
                    return False
        ...
        while True:
            ...
            if not work:
                ...
                if len(work) == 3:
                    ...
                    response = self.check(
                        hostname, record_type, timeout_retries)
                else:
...
def run(target, record_type=None, subdomains="names.txt", resolve_list="resolvers.txt", process_count=16):
    ...
    for i in range(process_count):
        worker = lookup(in_q, out_q, resolve_q, target,
                        wildcards, spider_blacklist)
        worker.start()
...
if __name__ == "__main__":
    ...
    for target in targets:
        ...
        if target:
            ...
            # options.output
            # options.json
            print(target, record_type, options.subs, options.resolvers,
